...
Detekcija i prepoznavanje objekata je jedna od ključmnih tehnika procesiranja sliak na tradicionalana način ali i pomoću neurosnkih mreža. Bazirano je na principu pronalaznka obijekta i crtanja okvirne kutije oko tog objekt \cite{segvsdetection, , segvsdetection_2}. 

To mu je ujedno i glavna razliak naspram segmentacije slike, segmentacija dodijeljuje klasu svakom piskelu koji odgovara onome što se traži, dok detekcija obijekta to ne radi već samo locira obijekt određene klase na slici \cite{segvsdetection, segvsdetection_2}.

Ponekad je samo informacija o lokaciji obijekta dovoljna, u slučaju da nije segmentacija nudi puno više informacija o samom objektu.

Da bi detektiranje objekata bilo uspješno često se koriste konture. Konture su ništa drugo nego krivulje koje opisuju rubove objekata, zbog toga su jako korisne kdo detektiranja oblika. Također se ponekad koriste i kdo segmentiranja.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{slike/segVsDetec.png}
    \caption{Usporedba segmentaicje i detekcije objekta \cite{segvsdetection_2}}
\end{figure}
